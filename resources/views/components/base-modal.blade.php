...
<div class="modal fade" id="{{ $modalId }}Modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
      <form class="modal-content" id="{{ $formId ?? '' }}" action="{{ $route ?? '' }}" method="POST" enctype="multipart/form-data">
        @csrf
        @if ($type == 'Edit')
            @method("PUT")
        @endif
        <div class="modal-header">
          <h5 class="modal-head" id="{{ $modalId }}ModalTitle">{{ $type }} Data</h5>
          <button
            type="button"
        ...
        <div class="modal-body">
            <div class="flex loader-container justify-center my-5">
        ...
        @if ($type != 'Detail')
            <div class="hidden footer justify-content-center gap-3 mb-4 w-100 px-4">
                <button type="submit" class="button btn-main w-100">{{ $type }} Data</button>
